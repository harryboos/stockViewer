...
「预测」Tab 仅有一个模块，固定使用 **GLM 5.3 MAX**（`glm-5.3`、`thinking.type=enabled`、`reasoning_effort=max`）。窗口为北京时间生成日次日起的 **15 个自然日**，含周末、节假日；例如 9 月 12 日生成时预测区间为 9 月 13–27 日。仅点击生成才调用模型与联网检索；切换 Tab 只读取状态，结果按生成日独立缓存，不覆盖板块概念推荐或每日选股。
...
提示词核心是：比较趋势延续、回调修复和催化轮动机会，解释「为什么现在、为什么在未来半个月可能体现」，用现实事件→供需／价格／成本／订单→盈利预期的链条说明原因，主动考虑反面证据，并给出可观察的确认与失效条件。完整提示词见 `backend/concept_forecast.py` 的 `SYSTEM_INSTRUCTION` 和 `build_prompt()`。每次调用会附上真实候选材料与严格的输出结构。预测任务单独使用 `forecast:glm` 运行锁，与概念推荐一样最多分析 10 分钟、锁保留 11 分钟；无需迁移数据库或增加配置项。
