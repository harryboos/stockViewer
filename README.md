...
推荐在当日概念强度榜和成交额榜中最多 8 个活跃概念内比较，要求近 5 个交易日涨幅为正，并有同一交易日的有效历史与强势成份股。每个概念展示当日涨跌、成交额、主力净流入、上涨广度及家数、近 5/10 日累计涨幅，以及最多 3 只强势股的价格、涨幅、成交额、换手率和理由。成份股候选取涨幅前 100 名中当日上涨、成交额至少 1 亿元的正常股票，排除 ST/退市标签；历史不足或报价过期时不会补造行情。

影响因素区分「行情依据」「资讯线索」「待验证推测」，资讯来自东方财富近 7 天搜索结果并附原文链接、来源与时间；没有相关资讯会明确提示。AI 只能引用输入的概念、成份股和来源 ID，展示数字由服务端原始数据填充。免费行情可能延迟、受限或缺失；非交易日明确展示最近交易日日期。历史行情和板块快照的采集时刻可能略有差异，资讯相关性也不等于已证实的因果关系。

生成任务在后台运行，150 秒超时后可重试；数据库原子运行锁防止并发重复调用。分析缓存使用独立命名空间，复用现有 SQLite 数据库，无需迁移或安装新依赖。
